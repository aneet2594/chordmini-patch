...
def validate_audio_file(audio_path: str) -> bool:
    ...
    Relaxed validation — skips librosa.load (which requires ffmpeg in PATH
    for MP3 files and can fail even for valid files in some Docker environments).
    Actual corruption will be caught by the detector at processing time.
    """
    import os
    try:
